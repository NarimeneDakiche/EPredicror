 ...
 * @author Account
 */
public class CPM_old implements LongTask {
    ...
    public LinkedList<LinkedList<Node>> execute(GraphModel gm, boolean directed, int k) {
        System.out.println("CPM started...");
        ...
        for (Node vi : nodes) {
            for (Node vj : nodes) {
                if (!(vi.getId().toString().equals(vj.getId().toString())) && (getSharedNodes(vi, vj) == k - 1)) {
                    if (g.isDirected()) {
        ...
        System.out.println("CPM done. " + cp.computeWeeklyConnectedComponents(g, cp.createIndiciesMap(g)).size() + " partitions found\n");
